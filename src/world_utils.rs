...
///
/// Generates a unique "Lukas Welt N" name, creates the directory structure
...
pub fn create_new_world(base_path: &Path) -> Result<String, String> {
    ...
    // Check for both "Lukas Welt X" and "Lukas Welt X: Location" patterns
    let mut counter: i32 = 1;
    let unique_name: String = loop {
        let candidate_name: String = format!("Lukas Welt {counter}");
        ...
        // Check for worlds with location suffix (Lukas Welt X: Location)
        let location_pattern = format!("Lukas Welt {counter}: ");
